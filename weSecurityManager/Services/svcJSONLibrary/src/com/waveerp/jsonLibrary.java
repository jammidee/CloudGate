...
/**
 * 
 * Jammi Dee 05/03/2012
 * 05:45
 * Modified: 06/19/2012 10:40
 * This is a manual generation of JSON
 * 
 */
...
public class jsonLibrary extends com.wavemaker.runtime.javaservice.JavaServiceSuperClass {
    ...
    public String loadData(String treeType, String sqlScript, String strParent, String strDesc){
        ...
        // "0" is the initial eid of 000000

        try
        {
            
           strBug = "[" + generateTree( strParent , strDesc , "0") + "]" ;
    ...
    public String generateTree(String strParent, String strDesc, String strIcon ) throws Exception {  
        ...
        // Select the theme of the tree here
        if ( strTreeType.equals("USER") ){
            strBranch = "<image src='resources/images/buttons/tree-usergroup-icon.png'/>";
            strleaf = "<image src='resources/images/buttons/tree-user-icon.png'/>";
        } else if ( strTreeType.equals("FILE") ){
            strBranch = "<image src='resources/images/buttons/tree-filegroup-icon.png'/>";
            strleaf = "<image src='resources/images/buttons/tree-file-icon.png'/>";
        } else if ( strTreeType.equals("TABLE") ){
            strBranch = "<image src='resources/images/buttons/tree-tablegroup-icon.png'/>";
            strleaf = "<image src='resources/images/buttons/tree-table-icon.png'/>";
        }  else {
            strBranch = "<image src='resources/images/buttons/tree-filegroup-icon.png'/>";
            strleaf = "<image src='resources/images/buttons/tree-file-icon.png'/>";
        }
        ...
        strHead         =  "{\"content\":\"" + strBranch + " " + strDesc + "\", \"eid\":\"" + strIcon + "\", ";
        strSubHead      = ""; 
        strJSON         = "undefined";
        strTail         = "}";
        strSubTail      = "";
        ...
        if( checkChild(strParent) > 0 ){
            ...
            for (int j=0 ; j <= nodid.length - 1 ; j++  ){
                ...
                if ( checkChild(nodid[j]) > 0 ) {
                    ...
                    if ( j == nodid.length - 1 ) {
                        strJSON = strJSON + generateTree(nodid[j], noddesc[j], nodid[j] ) + "";
                    } else {
                        strJSON = strJSON + generateTree(nodid[j], noddesc[j], nodid[j] ) + ",";
                    }    
                    ...
                    if (  j == nodid.length - 1  ) {
                        strJSON = strJSON + "{\"content\":\"" + strleaf + " " + noddesc[j] + "\", \"eid\":\"" +  nodid[j] + "\" }";
                    } else {
                        strJSON = strJSON + "{\"content\":\"" + strleaf + " " + noddesc[j] + "\", \"eid\":\"" + nodid[j] + "\" },";
                    }
